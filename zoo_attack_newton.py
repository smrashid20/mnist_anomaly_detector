...
def zoo_attack(network, image, t_0, fw, step_size):
    ...
    while max_iteration > 0 and not convergence:
        ...
        if h_i <= 0:
            delta_star = -step_size * g_i
        else:
            delta_star = -step_size * (g_i / h_i)
...
total = 0
success = 0
num_image = 3  # number of images to be attacked
...
for i, (images, labels) in enumerate(testloader):
    ...
    for step_size in step_sizes:
        with open('output_logs_newton/log_{}_{}.txt'.format(step_size, i), 'w') as fw:
            ...
            if adv_pred.item() != labels.item():
                ...
                success += 1
                save_image(adv_image, 'output_images_newton/image_{}_{}'.format(step_size, i))
